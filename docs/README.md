...
[getting started](using/getting-started.md) for what it emits.
...
- **[Using tenon](using/)**: task-oriented guides for building queries.
  [getting started](using/getting-started.md) (generating a schema and
  connecting), [queries](using/queries.md),
  [joins](using/joins.md), [writes](using/writes.md),
  [relationships](using/relationships.md), and the
  [type mapping](using/type-mapping.md) between Postgres and TypeScript.
- **[How tenon works](internals/)**: the internals, for working on
  tenon itself. Start with [the query pipeline](internals/pipeline.md):
  a line-by-line trace of what happens when you run a query, from the
  fluent call to the rows coming back.
...
changes. See [the architecture overview](internals/).
