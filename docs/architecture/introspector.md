...
generate.ts: GenerateOptions  →  open pg.Client
    |
    v
readCatalog.ts: pg_catalog queries  →  { columns, foreignKeys, primaryKeys }
    |
    v
...
A third query reads primary-key constraints from the same
`pg_constraint` table:
...
Same `generate_subscripts` + `array_agg` pattern as the FK
query, but only one side: each constraint produces its
referencing columns. Composite PKs come back with
`columns.length > 1`. Tables without a declared primary key
produce no row at all — the emitter omits the trailing argument
to `defineTable` for those.
...
Pure: takes `CatalogColumn[]`, `CatalogForeignKey[]`, and
`CatalogPrimaryKey[]`, returns a `string`. The caller writes it
to disk.

The whole file body is a single `defineSchema(...)` call wrapping
one `defineTable(...)` per group, destructured back to per-table
exports:

```ts
export const schema = defineSchema({
  <exportName>: defineTable(<schema>, <table>, {
...
The destructured exports inherit the wired `find` types from
`defineSchema`, so user code keeps importing `posts` / `users`
and gets the FK accessors automatically. `defineSchema` mutates
each Table's `find` in place at runtime — see
[`fluent-layer.md`](fluent-layer.md#schema-wide-wiring-defineschema)
for why.
...
The fourth and fifth `defineTable` arguments (FKs and primary
key) are emitted only when present. Four shapes:

- **Neither**: `defineTable("public", "events", { ... })`.
- **PK only**: `defineTable("public", "events", { ... }, [], { columns: ["id"] })`
  — the FKs slot is `[]` because positional arguments require
  it.
- **FKs only**: `defineTable("public", "posts", { ... }, [...fks])`.
- **Both**: `defineTable("public", "posts", { ... }, [...fks], { columns: ["id"] })`.

Composite primary keys are emitted faithfully (their `columns`
array has more than one entry), but `Table.find(id)` is omitted
from the resulting Table type — composite-PK find is deferred.

Composite FKs get a two-line generated comment above the table
entry:
